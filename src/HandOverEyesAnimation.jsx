...
export default function App() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Arm raise animation
      const progress = (Math.sin(t * 1) + 1) / 2;
      const eased = progress * progress * (3 - 2 * progress);

      const shoulderAngle = 120 + eased * 30;  // 120 → 150 deg
      const elbowAngle = 30 + eased * 80;      // 30 → 110 deg

      leftUpper.current.style.transform = `rotate(${shoulderAngle}deg)`;
      leftLower.current.style.transform = `rotate(${elbowAngle}deg)`;
      rightUpper.current.style.transform = `rotate(${-shoulderAngle}deg)`;
      rightLower.current.style.transform = `rotate(${-elbowAngle}deg)`;
...
const scaleWrapper = {
  ...
  alignItems: "center",
  justifyContent: "center", // ensures the cat "sits" on the bottom
  height: "100%",             // fills the stage vertically
  width: "100%",              // fills the stage horizontally
};

const scaleContainer = {
  transform: "scale(1.5)",    // scale cat+shadow
  display: "flex",
  flexDirection: "column",
  alignItems: "center",
};
...
const upperArmLeft = { position: "absolute", top: 120, left: 50, width: 18, height: 70, background: "linear-gradient(#b9682c,#6b3a14)", borderRadius: 20, transformOrigin: "top center", transform: "rotate(120deg)" };
const upperArmRight = { position: "absolute", top: 120, right: 50, width: 18, height: 70, background: "linear-gradient(#b9682c,#6b3a14)", borderRadius: 20, transformOrigin: "top center", transform: "rotate(-120deg)" };
const lowerArm = { position: "absolute", top: 70, left: 0, width: 18, height: 50, background: "linear-gradient(#b9682c,#6b3a14)", borderRadius: 20, transformOrigin: "top center", transform: "rotate(30deg)" };
const paw = { position: "absolute", bottom: -6, width: 28, height: 22, background: "#e8b98a", borderRadius: 14 };
const upperLegLeft = { position: "absolute", bottom: 0, left: 50, width: 24, height: 70, background: "linear-gradient(#6b3a14,#3e210d)", borderRadius: 20, transformOrigin: "top center", transform: "rotate(0deg)" };
const upperLegRight = { position: "absolute", bottom: 0, right: 50, width: 24, height: 70, background: "linear-gradient(#6b3a14,#3e210d)", borderRadius: 20, transformOrigin: "top center", transform: "rotate(0deg)" };
...
const lowerLeg = { position: "relative", top: "100%", left: 0, width: 24, height: 45, background: "linear-gradient(#6b3a14,#3e210d)", borderRadius: 20, transformOrigin: "top center", transform: "rotate(0deg)" };
const rearPaw = { position: "absolute", bottom: -6, left: "50%", transform: "translateX(-50%)", width: 30, height: 18, background: "#e8b98a", borderRadius: 12, zIndex: 4, display: "flex", justifyContent: "center", alignItems: "center", gap: 4 };
const rearClaw = { width: 4, height: 6, background: "#dcdcdc", borderRadius: 2, transform: "rotate(-10deg)" };
const tailStyle = { position: "absolute", bottom: 70, left: 100, width: 22, height: 135, background: `linear-gradient(180deg,#c07a3a 0%,#a5622a 40%,#8a4f1d 75%,#5a2f10 100%)`, borderTopLeftRadius: 18, borderTopRightRadius: 18, borderBottomLeftRadius: 14, borderBottomRightRadius: 14, transformOrigin: "bottom center", boxShadow: `inset -4px 0 6px rgba(0,0,0,0.25), inset 3px 0 4px rgba(255,255,255,0.08)`, zIndex: -1, willChange: "transform" };
...
const whisker = { width: 40, height: 1.5, background: "linear-gradient(to right, #f5f5f5, #cfcfcf)", borderRadius: "50%", borderTopLeftRadius: "100%", borderBottomLeftRadius: "100%", opacity: 0.55 };
const shadow = {
  position: "absolute",
  bottom: -60,
  left: "50%",
